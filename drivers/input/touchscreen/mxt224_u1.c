...
extern void flash_led_buttons(unsigned int);
...
static void report_input_data(struct mxt224_data *data)
{
	...
	for (i = 0; i < data->num_fingers; i++) {
		...
		// slide2invert gesture start
		if (copy_data->touch_is_pressed_arr[i] == 1 && copy_data->mxt224_enabled) {
			if (max(data->fingers[0].x, data->fingers[1].x) < x_lo) {
				printk(KERN_ERR "[TSP] slide2invert down at: %4d\n",
					data->fingers[i].x);
				invert_start = 1;
			}
		}
		...
		if (data->fingers[i].z == TSP_STATE_RELEASE) {
			...
			// slide2invert trigger
			if (invert_start == 1 && data->fingers[1].x > x_hi) {
				printk(KERN_ERR "[TSP] slide2invert up at: %4d\n",
					data->fingers[1].x);
